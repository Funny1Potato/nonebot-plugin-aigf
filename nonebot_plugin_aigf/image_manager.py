...
class ImageManager:
    ...
    async def get_image_description(self, image_base64: str, is_sticker: bool) -> ImageWithDescription | None:
        ...
        if image_format.upper() == "GIF":
            ...
            if not gif_b64:
                return None
            description = await self._vlm.request("用中文简短描述这张动态图的内容。如果图中有人物，只描述人物的外貌特征（发色、表情、动作、服装等），不要猜测或识别具体角色名称。若图中有文字，请尽量完整描述，若文字过多，可概括大意。", gif_b64, "jpeg")
            emotion = await self._vlm.request("用3个词概括这个表情包的情感，用顿号分隔，如：开心、得意、搞笑", gif_b64, "jpeg")
        else:
            description = await self._vlm.request("用中文简短描述这张图片的内容。如果图中有人物，只描述人物的外貌特征（发色、表情、动作、服装等），不要猜测或识别具体角色名称。若图中有文字，请尽量完整描述，若文字过多，可概括大意。", image_base64, image_format)
